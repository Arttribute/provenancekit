...
export default async function ProjectPage({ params }: Props) {
  ...
  const [keys, usage] = await Promise.all([
    getProjectApiKeys(project.id),
    getProjectUsageSummary(project.id),
  ]);
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            ...
            <p className="text-2xl font-bold">{usage.successRate.toFixed(1)}%</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" asChild>
                <Link href={`/${orgSlug}/${projectSlug}/api-keys`}>
                  Manage →
                </Link>
              </Button>
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Explore resources and attribution graphs
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-0 space-y-2">
            <Button variant="outline" className="w-full justify-start" asChild>
              <Link href={`/${orgSlug}/${projectSlug}/resources`}>
                <Database className="h-4 w-4 mr-2" />
                Browse Resources
              </Link>
            </Button>
            <Button variant="outline" className="w-full justify-start" asChild>
              <Link href={`/${orgSlug}/${projectSlug}/provenance`}>
                <GitBranch className="h-4 w-4 mr-2" />
                Provenance Graph
              </Link>
            </Button>
            <Button variant="outline" className="w-full justify-start" asChild>
              <Link href={`/${orgSlug}/${projectSlug}/analytics`}>
