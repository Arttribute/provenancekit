...
2. **File not yet in the system** — ask the user if they own it before recording:
   - **Owned** → `action.type = "create"` → enters as a claimed resource
   - **Not owned** → `action.type = "reference"` → enters as an unclaimed/external source

The `FileProvenanceTag` + `FileOwnershipClaim` UI components from `@provenancekit/ui` handle this flow automatically.

See [File Attribution & Ownership Claims](/patterns/file-attribution) for full implementation details.
